...
const Scene = dynamic(() => import("./Scene").then((mod) => mod.default), {
  ssr: false,
  loading: () => <div className="h-[45dvh] w-full bg-[#000205]" />,
});
...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-[100dvh] w-full flex flex-col bg-[#000205] overflow-x-hidden">
      {/* 3D GLOBE AREA - Increased height slightly for mobile impact */}
          ...
          <div className="flex flex-col space-y-6 lg:space-y-8 text-center lg:text-left items-center lg:items-start pt-6 lg:pt-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-xl text-left">
              {services.map((service, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 border border-white/5 bg-white/[0.03] p-3 rounded-xl backdrop-blur-sm transition-transform active:scale-95"
                >
            ...
            <div className="pt-4 w-full sm:w-auto">
              <Link href="/contact">
                <button className="bg-white text-[#000814] px-10 py-5 rounded-2xl font-black text-sm tracking-widest hover:bg-[#3a86ff] hover:text-white transition-all w-full sm:min-w-[280px] active:scale-95 shadow-2xl shadow-white/5">
